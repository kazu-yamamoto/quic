  ...
  where
    ...
    detectLossUpdateCC newlyAckedPackets = case Seq.viewr newlyAckedPackets of
        ...
        _ :> lastPkt -> do
            ...
            when
                ( spPacketNumber lastPkt == largestAcked
                    && any spAckEliciting newlyAckedPackets
                )
                $ do
            ...
            unless (null lostPackets) $ do
                ...
                if lvl == RTT1Level && mode /= SlowStart
                    then mergeLostCandidates ldcc lostPackets
                    else do
                        -- just in case
